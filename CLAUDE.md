...
`public.rs` and `precons.rs` reuse (see `decks::{legality,bracket,stats,goldfish,tokens,combos,mana,pricing,roles}`, and
`decks::add_to_collection`, which takes the full write path the same way), the
